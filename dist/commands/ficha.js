...
exports.handleFichaSubmit = handleFichaSubmit;
exports.handleFichaInteraction = handleFichaInteraction;
const discord_js_1 = require("discord.js");
const FichaSchema_1 = require("../models/FichaSchema");
const OCSchema_1 = require("../models/OCSchema");
function compilarTemplate(rawText) {
    ...
    for (const line of lines) {
        ...
        if (rules.includes('|int|'))
            ...
            type = "float";
        else if (rules.includes('|image|'))
            type = "image";
        else if (rules.includes('|if|'))
            type = "if";
        let options = [];
        if (type === "if") {
        ...
        switch (action) {
            case "create": {
                if (args[1]?.toLowerCase() !== "new")
                    return message.reply("Uso: `rp!ficha create new`");
                ...
                if (!rawContent || rawContent.trim() === "END")
                ...
                try {
                    ...
                    await FichaSchema_1.TemplateModel.findOneAndUpdate({ guildId }, { guildId, rawText: finalContent, fields: compiledFields }, { upsert: true });
            ...
            case "check": {
                ...
                await FichaSchema_1.TemplateModel.findOneAndUpdate({ guildId }, { checkChannelId: targetChannel.id }, { upsert: true });
                return message.reply(`✅ Canal de **Aprovação (Check)** definido para <#${targetChannel.id}>.`);
            }
            case "hall": {
                const targetChannel = message.mentions.channels.first() || message.channel;
                await FichaSchema_1.TemplateModel.findOneAndUpdate({ guildId }, { hallChannelId: targetChannel.id }, { upsert: true });
                return message.reply(`🏛️ Canal do **Hall da Fama** definido para <#${targetChannel.id}>.`);
            }
            case "oc": {
                if (args[1]?.toLowerCase() !== "on")
                    return message.reply("Uso: `rp!ficha oc on \"Prefixo: |prefix|\"`");
                const match = message.content.match(/["'](.*?)["']/);
                if (!match)
                    return message.reply("❌ Você precisa colocar o formato do prefixo entre aspas! Ex: `rp!ficha oc on \"Prefixo: |prefix|\"`");
                const appendedText = match[1];
                const prefixLabel = appendedText.split(':')[0].trim();
                const t = await FichaSchema_1.TemplateModel.findOne({ guildId });
                if (!t)
                    return message.reply("❌ Crie um template primeiro com `rp!ficha create new` antes de ativar o OC!");
                ...
                try {
                    ...
                    await FichaSchema_1.TemplateModel.findOneAndUpdate({ guildId }, {
                        rawText: newRawText,
            ...
            case "edit": {
                const t = await FichaSchema_1.TemplateModel.findOne({ guildId });
                if (!t)
                    return message.reply("❌ Não existe template para editar. Use `create new`.");
                return message.reply(`✏️ **Template Atual:**\n\`\`\`text\n${t.rawText}\n\`\`\`\nUse \`rp!ficha create new\` para sobrescrever.`);
            }
            case "end": {
                const name = args.slice(1).join(" ").replace(/['"]/g, '');
                if (!name)
                    return message.reply("Uso: `rp!ficha end \"Nome do Personagem\"`");
                const deleted = await FichaSchema_1.FichaModel.findOneAndDelete({ guildId, characterName: new RegExp(`^${name}$`, 'i') });
                if (!deleted)
                ...
                if (deleted.hallMessageId) {
                    ...
                    if (t && t.hallChannelId) {
                        try {
                            const hallChan = await message.guild.channels.fetch(t.hallChannelId);
            ...
            case "purge": {
                await FichaSchema_1.FichaModel.deleteMany({ guildId, status: "approved" });
                return message.reply("🔥 **PURGE CONCLUÍDO.** Todas as fichas aprovadas foram obliteradas.");
            }
            case "reset": {
                await FichaSchema_1.TemplateModel.findOneAndDelete({ guildId });
                await FichaSchema_1.FichaModel.deleteMany({ guildId });
...
async function handleFichaSubmit(message) {
    if (message.author.bot || message.content.startsWith("rp!"))
        return false;
    if (!message.guild)
        return false;
    const guildId = message.guild.id;
    const template = await FichaSchema_1.TemplateModel.findOne({ guildId });
    if (!template || !template.fields.length)
        return false;
    const firstField = template.fields[0].name;
    if (!message.content.toLowerCase().includes(`${firstField.toLowerCase()}:`))
        return false;
    let parsedData = {};
    ...
    for (let i = 0; i < template.fields.length; i++) {
        ...
        if (currentField.type === "int") {
            ...
            if (isNaN(num)) {
                message.reply(`❌ O campo **${currentField.name}** precisa ser um número inteiro!`);
                return true;
            }
            ...
            if (isNaN(num)) {
                message.reply(`❌ O campo **${currentField.name}** precisa ser numérico!`);
                return true;
            }
            parsedData[currentField.name] = num;
        }
            ...
            for (const opt of validOptions) {
                if (opt.val.toLowerCase() === userChoice) {
                    if (opt.dep) {
                        ...
                        if (opt.op === '>=' && depValue >= opt.limit)
                            isAllowed = true;
                        else if (opt.op === '<=' && depValue <= opt.limit)
                            isAllowed = true;
                        else
                            ruleBroken = `Requisito não atingido: ${opt.dep} precisa ser ${opt.op} ${opt.limit} para ser ${opt.val}.`;
                    }
                    else {
                        isAllowed = true;
                    }
    ...
    const fichaDoc = await FichaSchema_1.FichaModel.create({
        guildId, userId: message.author.id, characterName, data: parsedData, avatar
    });
    if (template.checkChannelId) {
        const checkChan = await message.guild.channels.fetch(template.checkChannelId);
        if (checkChan) {
            ...
            const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId(`ficha_approve_${fichaDoc._id}`).setLabel('Aprovar').setStyle(discord_js_1.ButtonStyle.Success), new discord_js_1.ButtonBuilder().setCustomId(`ficha_reject_${fichaDoc._id}`).setLabel('Recusar').setStyle(discord_js_1.ButtonStyle.Danger));
            await checkChan.send({ embeds: [embed], components: [row] });
        }
    }
    return true;
}
async function handleFichaInteraction(interaction) {
    ...
    const ficha = await FichaSchema_1.FichaModel.findById(fichaId);
    if (!ficha)
        return interaction.reply({ content: "Ficha não encontrada no banco.", ephemeral: true });
    if (action === "reject") {
        await FichaSchema_1.FichaModel.findByIdAndDelete(fichaId);
    ...
    if (action === "approve") {
        ...
        const template = await FichaSchema_1.TemplateModel.findOne({ guildId: interaction.guildId });
        if (template?.hallChannelId) {
            const hallChan = await interaction.guild.channels.fetch(template.hallChannelId);
            if (hallChan) {
                const embed = new discord_js_1.EmbedBuilder()
