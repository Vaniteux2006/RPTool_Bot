...
exports.default = {
    ...
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('ver')
        .setDescription('O que você quer ver?')
        .addChoices({ name: '🏰 Geral (Tudo)', value: 'geral' }, { name: '📸 Foto do Server', value: 'photo' })),
    async executeSlash(interaction) {
        ...
        if (modo === 'photo') {
            ...
            if (icon)
                ...
                await interaction.reply("Este servidor não tem ícone.");
            return;
        }
        const fakeMessage = {
    ...
    async execute(message, args) {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: '🆔 Identidade', value: `**ID:** \`${guild.id}\`\n**Dono:** ${owner.user.tag}\n**Criado em:** ${criacao}`, inline: false }, { name: '📊 Estatísticas', value: `**Membros:** ${guild.memberCount}\n**Bans:** ${banCount}`, inline: true }, { name: '💬 Canais', value: `**Total:** ${totalChannels}\n**Texto:** ${textChannels} | **Voz:** ${voiceChannels}`, inline: true }, { name: '👴 Ancião do Server', value: oldMember ? `${oldMember.user.tag} (Entrou em <t:${Math.floor(oldMember.joinedTimestamp / 1000)}:d>)` : "Nenhum (Só tem o dono ou bots)", inline: false });
        if (guild.bannerURL()) {
