...
async function chamarIAResumo(prompt, config) {
    try {
        if (!config || !config.provider)
            throw new Error("Configuração de IA inválida ou incompleta.");
...
exports.command = {
    ...
    execute: async (message, args) => {
        const aiConfig = await (0, tokenHelper_1.getGuildAIConfig)(message.guild.id);
        if (!aiConfig) {
...
Responda APENAS com o texto formatado conforme solicitado.`;
            ...
            const embedPage1 = new discord_js_1.EmbedBuilder()
                .setColor(0x5865F2)
            ...
            const embedPage2 = new discord_js_1.EmbedBuilder()
                .setColor(0x5865F2)
            ...
            collector.on('collect', async (i) => {
                ...
                if (i.customId === 'page_1') {
                    row.components[0].setDisabled(true).setStyle(discord_js_1.ButtonStyle.Secondary);
                    row.components[1].setDisabled(false).setStyle(discord_js_1.ButtonStyle.Primary).setLabel('Saiba Mais ➜');
                    await i.update({ embeds: [embedPage1], components: [row] });
                }
                else if (i.customId === 'page_2') {
                    row.components[0].setDisabled(false).setStyle(discord_js_1.ButtonStyle.Secondary);
                    row.components[1].setDisabled(true).setStyle(discord_js_1.ButtonStyle.Primary).setLabel('Detalhes (Atual)');
