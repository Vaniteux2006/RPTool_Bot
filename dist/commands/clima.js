...
async function getWeatherInstant(lat, lon) {
    try {
...
async function getWeatherHistorical(lat, lon, date) {
    try {
...
exports.default = {
    ...
    data: new discord_js_1.SlashCommandBuilder().setName('clima').setDescription('Comandos de clima'),
    async execute(message, args) {
        let ClockModel;
        try {
            ClockModel = database_1.mainConnection.model('Clock');
        }
        catch (e) {
            return message.reply("❌ Use `rp!time` primeiro para inicializar o sistema.");
        }
        if (args.length === 0) {
            return message.reply("🌦️ **Sistema Climático**\n" +
        ...
        if (action.startsWith('<#')) {
            ...
            if (!clock)
                return message.reply("❌ Não há relógio ativo neste canal.");
            ...
            if (clock.forcedWeather) {
                ...
                if (isStandard)
                    climaTexto = `⚠️ **${clock.forcedWeather}** (Manual)`;
                else
                    climaTexto = `🚫 **${clock.forcedWeather}** (Anomalia)`;
            }
            ...
            return message.reply(`🌍 **Clima em ${localNome}** (Data RP: ${dateStr})\n` +
                `${climaTexto}`);
        }
        if (action === 'def') {
            const list = Object.values(WMO_TABLE).map(c => `• ${c}`).join('\n');
            return message.reply(`📜 **Climas Padrão (WMO):**\n${list}\n\n` +
                `*Qualquer condição fora desta lista será tratada como Anomalia (🚫).*`);
        }
        if (action === 'force') {
            const clockName = args[1];
            const condition = args.slice(2).join(' ');
            if (!clockName || !condition)
                return message.reply("❌ Uso: `rp!clima force <Relógio> <Texto>`");
        ...
        if (action === 'sync') {
            ...
            if (!clockName || !query)
                return message.reply("❌ Uso: `rp!clima sync <Relógio> <Lugar>`");
            const msg = await message.reply("🔍 Buscando...");
            const geo = await getCoords(query);
            if (!geo)
                return msg.edit("❌ Local não encontrado.");
            const res = await ClockModel.updateOne({ name: clockName }, { latitude: geo.lat, longitude: geo.lon, locationName: `${geo.name}, ${geo.country}`, forcedWeather: null });
            if (res.matchedCount === 0)
                return msg.edit("❌ Relógio não encontrado.");
            return msg.edit(`🌍 **${clockName}** agora está em **${geo.name}**.`);
        }
        const query = args.join(' ');
        const geo = await getCoords(query);
        if (!geo)
            return message.reply(`❌ Local não encontrado: "${query}"`);
        const weather = await getWeatherInstant(geo.lat, geo.lon);
        if (!weather)
            return message.reply("❌ Erro na API.");
        message.reply(`📍 **${geo.name}**\n` +
            `🌡️ **${weather.temp}°C**\n` +
            `👀 **${wmoToText(weather.code)}**\n` +
            `-# (Agora / Tempo Real)`);
    },
    async executeSlash(i) { i.reply({ content: "Use rp!clima", ephemeral: true }); }
};
