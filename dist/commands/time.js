...
exports.ClockModel = database_1.mainConnection.models.Clock || database_1.mainConnection.model('Clock', ClockSchema);
...
function calculateMultiplier(args, symbol) {
    ...
    if (!fromStr || arrow !== '->' || !toStr)
        return 1;
    const fromMs = parseTimeStr(fromStr);
    const toMs = parseTimeStr(toStr);
    return fromMs === 0 ? 1 : toMs / fromMs;
}
function formatClockDisplay(clock, rpgDate, earthDate, weatherData) {
    const timeZone = 'UTC';
    const timeOptions = { timeZone, hour: '2-digit', minute: '2-digit' };
    const dateExtensoOptions = { timeZone, weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' };
    const dateCurtaOptions = { timeZone, day: '2-digit', month: '2-digit', year: 'numeric' };
...
exports.default = {
    ...
    data: new discord_js_1.SlashCommandBuilder().setName('time').setDescription('Use rp!time'),
    async checkAndRestoreClocks(client) {
        if (isEngineRunning)
            return;
    ...
    async updateClocks(client) {
        try {
            const clocks = await exports.ClockModel.find({});
            for (const clock of clocks) {
                try {
                    const channel = await client.channels.fetch(clock.channelId);
                    if (!channel)
                        continue;
                    ...
                    console.error(`Erro clock ${clock.name}:`, e);
                }
            }
        }
        catch (err) {
    ...
    async execute(message, args) {
        if (args.length === 0) {
            return message.reply("🕰️ **Sistema Temporal - Guia de Comandos**\n\n" +
                "🔹 **Básico**\n" +
                "`rp!time #canal [Fuso]`\n" +
        ...
        if (action.startsWith('<#')) {
            ...
            await exports.ClockModel.deleteOne({ name: autoName });
            const msg = await ch.send("⏳ Iniciando relógio...");
            await exports.ClockModel.create({
        ...
        if (action === 'set') {
            ...
            await exports.ClockModel.deleteOne({ name });
            ...
            const target = args[1];
            await exports.ClockModel.deleteOne({ name: target });
            message.reply("🗑️ Deletado.");
        }
        else if (action === 'skip') {
            const name = args[1];
            const amount = parseTimeStr(args[2]);
            const clock = await exports.ClockModel.findOne({ name });
            if (clock) {
                const now = Date.now();
                ...
                message.reply("❌ Não achei o relógio.");
        }
        else if (action === 'list') {
            const all = await exports.ClockModel.find({});
            message.reply(all.map(c => `**${c.name}** em <#${c.channelId}>`).join('\n') || "Nenhum relógio ativo.");
        }
    },
    async executeSlash(i) { i.reply({ content: "Use rp!time", ephemeral: true }); }
};
